...
//const DGRAPH_ENDPOINT = 'https://alpha-acct-test.tekit.io';
...
const getQueryByType = (limit, offset) => `
  query bytype($value: string!){ 
    typed as var(func: has(hash)) @cascade { 
      content_groups { 
        contents  @filter(eq(label,"type") and eq(value, $value)){
        } 
      } 
    }
...
export const queryCustom = async (query) => {
  return await client.newTxn({ readOnly: true }).query(query);
}

changeURL(DGRAPH_ENDPOINT);
